...
class AdvancedEnsembleDetector:
    ...
    def __init__(self):
        ...
        self.high_confidence_patterns = {
            ...
            'LLM06_Info_Disclosure': [
                ('training data', 1.0),  # Higher for info disclosure
                ('system prompt', 1.0),
                ('password', 1.0),
                ('api key', 1.0),
                ('secret', 1.)
    ...
    def test_detection_directly(self, prompt: str):
        ...
        try:
            ...
            return {"error": str(e)}
    def detect_attack_with_business_logic(self, text: str, context: Dict = None) -> Dict[str, Any]:
        """Enhanced detection with business logic for info disclosure"""
        assistant_response = context.get('assistant_response', '') if context else ''
        
        # First, get the raw detection result
        raw_result = self.detect_attack(text, context)
        
        # Apply business logic for information disclosure scenarios
        if raw_result['prediction'] == 'LLM06_Info_Disclosure':
            # Use the information disclosure scoper
            scoped_result = self.info_scoper.classify_information_scenario(
                text, assistant_response, 
...
class CVSS4Scorer:
    ...
    def assess_safety_impact(self, 
        ...
        # CRITICAL FIX: If assistant refused an information disclosure attempt
        if prediction == 'LLM06_Info_Disclosure':
            # Check for refusal patterns in assistant response
            refusal_patterns = [
                'cannot', 'sorry', 'unable', 'not allowed', 'against',
                'policy', 'confidential', 'i cannot', 'i\'m sorry', 'i am not'
            ]
            
            refusal_detected = any(pattern in response_lower for pattern in refusal_patterns)
            
            if refusal_detected:
                # ✅ ATTEMPT ONLY - Assistant refused = LOW safety impact
                logger.info(f"✅ Information disclosure ATTEMPT (refused) → Safety Impact: LOW")
                return SafetyImpactLevel.LOW
            
            # Check if assistant actually leaked information
            leakage_indicators = [
                'training data', 'system prompt', 'password is', 'api key is',
                'secret is', 'here is', 'the password', 'the key', 'my configuration',
                'my system', 'internal', 'confidential'
            ]
            
            actual_leakage = any(indicator in response_lower for indicator in leakage_indicators)
            
            if actual_leakage:
                # ✅ ACTUAL LEAKAGE - Assistant revealed information = HIGH safety impact
                logger.info(f"🚨 Information disclosure SUCCESS (leaked) → Safety Impact: HIGH")
                return SafetyImpactLevel.HIGH
            else:
                # UNCLEAR - Couldn't determine outcome = NONE safety impact
                logger.info(f"⚠️ Information disclosure UNCLEAR → Safety Impact: NONE")
    ...
    def generate_cvss_vector(self, 
        ...
        # ✅ CRITICAL FIX: If prediction is LLM06 and safety impact is LOW
        # (meaning info disclosure attempt was REFUSED), reduce impact scores
        if prediction == 'LLM06_Info_Disclosure' and si == SafetyImpactLevel.LOW:
            # Attempt only, NO actual leakage - reduce confidentiality impact
            vector = vector.replace("VC:H", "VC:L").replace("SC:H", "SC:L")
            logger.info(f"✅ Adjusted CVSS vector for refused info disclosure: {vector}")
        ...
        if si == SafetyImpactLevel.HIGH:
            ...
            if prediction != 'LLM06_Info_Disclosure':  # Already handled above
