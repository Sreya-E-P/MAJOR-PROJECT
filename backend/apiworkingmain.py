...
PYRIT_DIR = Path(r"C:\Users\LENOVO\Desktop\llm_security_framework\PyRIT")
...
# ==================== ADVANCED ENSEMBLE DETECTOR (WITH TIMEOUT PROTECTION) ====================
class AdvancedEnsembleDetector:
    ...
    def detect_attack(self, text: str, context: Dict = None) -> Dict[str, Any]:
        """FIXED detection method with timeout protection"""
        
        start_time = time.time()
        
        # Quick fallback for very short texts to avoid model processing
        if len(text.strip()) < 5:
            return self._get_quick_fallback(text, start_time)
        
        try:
            logger.info(f"🔍 Starting detection for text: {text[:100]}...")
            
            if not self.models:
                logger.warning("⚠️ No models available, using fallback")
                return self._get_fallback_result(text, start_time)
            
            # Tokenize input with SHORT timeout
            try:
                encoding = self.tokenizer(
                    text,
                    truncation=True,
                    padding=True,
                    max_length=512,  # Increased for better accuracy
                    return_tensors='pt'
                )
            except Exception as e:
            ...
            # Get predictions from available models with TIMEOUT PROTECTION
            predictions = []
            models_used = []
            ...
            for name, model in self.models.items():
                try:
                    ...
                    with torch.no_grad():
                        ...
                        logger.info(f"✅ {name} completed: {self.label_map.get(predicted_class.item(), 'Unknown')} ({confidence.item():.3f})")
            ...
            final_prediction = self._simple_voting(predictions)
            
            # Add timing and metadata
            final_prediction['detection_time'] = time.time() - start_time
            final_prediction['models_used'] = models_used
            final_prediction['total_models'] = len(predictions)
            
            # Get MITRE ATT&CK mapping
            try:
                mitre_mapping = self.mitre_mapper.get_mitre_mapping(final_prediction['prediction'])
                final_prediction['mitre_attack'] = mitre_mapping
            except Exception as e:
                logger.warning(f"⚠️ MITRE mapping failed: {e}")
                final_prediction['mitre_attack'] = {}
            
            logger.info(f"✅ Detection completed in {final_prediction['detection_time']:.2f}s: {final_prediction['prediction']}")
    ...
    def _simple_voting(self, predictions: List[Dict]) -> Dict[str, Any]:
        """Simple majority voting for predictions"""
        # Count votes
        vote_counts = {}
        for pred in predictions:
            pred_class = self.label_map.get(pred['prediction'], 'Benign')
            if pred_class in vote_counts:
                vote_counts[pred_class] += pred['confidence']
            else:
                vote_counts[pred_class] = pred['confidence']
        ...
        return {
            ...
            'severity_score': self.severity_scores.get(prediction, 0.0),
            'risk_level': self._determine_risk_level(prediction, confidence),
            'evidence': f'Ensemble voting: {prediction}',
            'context_analysis': {},
            'threat_indicators': []
        }
    ...
    def _determine_risk_level(self, prediction: str, confidence: float) -> str:
        ...
        risk_levels = {
            'CRITICAL': ['LLM01_Prompt_Injection', 'LLM02_Insecure_Output', 'LLM06_Info_Disclosure', 'LLM08_Excessive_Agency'],
        ...
        for level, attacks in risk_levels.items():
            if prediction in attacks:
                if level in ['CRITICAL', 'HIGH'] and confidence > 0.7:
                    return level
                elif level in ['MEDIUM'] and confidence > 0.6:
                    return level
                elif level in ['LOW'] and confidence > 0.5:
                    return level
                else:
                    return 'INFO'
        return 'INFO'
...
# ==================== SECURITY TOOL MANAGER (NO TIMEOUTS) ====================
class SecurityToolManager:
    """Manager for Garak and PyRIT security tools with CLI execution - NO TIMEOUTS"""
    ...
    def __init__(self):
        ...
        logger.info("🔧 Security Tool Manager initialized")
    ...
    async def analyze_log_files(self, 
                              garak_log_path: str = None,
                              pyrit_log_path: str = None,
                              analysis_id: str = None) -> Dict[str, Any]:
        """
        Analyze uploaded log files from Garak and PyRIT
        """
        if analysis_id is None:
            analysis_id = f"log_analysis_{uuid.uuid4().hex[:8]}"
            
        logger.info(f"📊 Analyzing log files: {analysis_id}")
        
        try:
            garak_results = {}
            pyrit_results = {}
            
            # Analyze Garak log if provided
            if garak_log_path and os.path.exists(garak_log_path):
                garak_results = self._analyze_garak_log(garak_log_path, analysis_id)
                
            # Analyze PyRIT log if provided  
            if pyrit_log_path and os.path.exists(pyrit_log_path):
                pyrit_results = self._analyze_pyrit_log(pyrit_log_path, analysis_id)
    ...
    def _analyze_garak_log(self, log_path: str, analysis_id: str) -> Dict[str, Any]:
        """Analyze uploaded Garak log file"""
        try:
            with open(log_path, 'r', encoding='utf-8') as f:
                log_content = f.read()
            
            # Parse log for vulnerabilities
            vulnerabilities = []
            lines = log_content.split('\n')
            
            for line in lines:
                if any(keyword in line.lower() for keyword in ["vulnerability", "fail", "jailbreak", "injection"]):
                    vulnerabilities.append({
                        "type": "detected_in_log",
                        "severity": "medium",
                        "confidence": 0.6,
                        "description": "Security issue found in log analysis",
                        "evidence": line.strip(),
                        "source": "garak_log"
                    })
    ...
    def _analyze_pyrit_log(self, log_path: str, analysis_id: str) -> Dict[str, Any]:
        """Analyze uploaded PyRIT log file"""
        try:
            with open(log_path, 'r', encoding='utf-8') as f:
                log_content = f.read()
            
            # Parse log for attack results
            vulnerabilities = []
            attack_successful = False
            lines = log_content.split('\n')
            
            for line in lines:
                if any(keyword in line.lower() for keyword in ["success", "true", "bypass"]):
                    attack_successful = True
                    vulnerabilities.append({
                        "type": "successful_attack",
                        "severity": "high",
                        "confidence": 0.8,
                        "description": "Successful attack detected in log",
                        "evidence": line.strip(),
                        "source": "pyrit_log"
                    })
...
async def analyze_logs(request: LogAnalysisRequest):
    """Analyze uploaded Garak and PyRIT log files"""
    try:
        results = await security_tool_manager.analyze_log_files(
            request.garak_log_path,
            request.pyrit_log_path
        )
...
if __name__ == "__main__":
    ...
    print("⏰ WITH TIMEOUT PROTECTION - Fast and reliable inference")
    print("="*80)
